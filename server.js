 ...
 * Integrated improved 'No common availability' logic
...
<html><head><meta charset="UTF-8"><title>${title}</title><style>
    ...
    .gm-form button{padding:.5rem 1.5rem;border:none;border-radius:4px;background:linear-gradient(to right,#f7ca28,#fe574e);color:#000;}
    /* Effects animations omitted for brevity... */
    ...
    table{margin:2rem auto;width:90%;max-width:500px;border-collapse:collapse;}
    th{background:#1e1e1e;color:#EEE;padding:.75rem;border:1px solid #333;text-align:left;}
    td{padding:.5rem;border:1px solid #333;text-align:left;}
...
app.get('/gm/users', requireLogin, requireGM, (req, res) => {
  ...
  Object.entries(users).forEach(([u, d]) => {
    html += `<tr><td>${u}</td><td>${d.email}</td><td>${d.isGM ? 'GM' : 'Player'}</td>` +
            `<td><form method="POST" action="/gm/users/${u}/delete"><button>Delete</button></form></td></tr>`;
  });
...
app.post('/gm/users/:username/delete', requireLogin, requireGM, (req, res) => {
  ...
  Object.values(games).forEach(g => {
    g.players = g.players.filter(p => p !== uname);
    delete g.votes[uname];
  });
  saveJSON(USERS_FILE, users);
...
app.get('/game/:id', requireLogin, (req, res) => {
  ...
  const game = games[id];
  if (!game || (!req.user.isGM && !req.user.games.includes(id))) return res.status(403).send('Forbidden');

  const days = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'];
  let html = `<h1 class="effect-${game.effect || 'none'} game-title">Vote: ${game.name}</h1>`;
  html += `<form method="POST" action="/game/${id}/vote"><input name="name" value="${req.session.user}" readonly><div class="days">`;
  days.forEach(d => {
    html += `<label><input type="checkbox" name="days" value="${d}"><span>${d}</span></label>`;
  });
...
app.get('/gm/create', requireLogin, requireGM, (req, res) => {
  const html = `<h1>Create Game</h1>` +
               ...
               `<select name="effect">` +
               `<option value="none">None</option><option value="snow">Snow</option>` +
               `<option value="electricity">Electricity</option><option value="glitch">Glitch</option>` +
               `<option value="swords">Swords</option></select>` +
...
app.post('/gm/create', requireLogin, requireGM, (req, res) => {
  ...
  games[gid] = { name: req.body.name, owner: req.session.user, votes: {}, players: [], effect: req.body.effect || 'none' };
...
app.get('/gm/game/:id', requireLogin, requireGM, (req, res) => {
  const id = req.params.id;
  const game = games[id];
  if (!game || game.owner !== req.session.user) return res.status(403).send('Forbidden');

  const votesObj = game.votes || {};
  const common = intersect(Object.values(votesObj));

  let html = `<h1 class="effect-${game.effect || 'none'} game-title">${game.name} (GM)</h1><hr>`;
  ...
  // Players list
  html += `<h2 class="game-title">Players</h2><ul>`;
  Object.entries(votesObj).forEach(([u, ds]) => html += `<li>${u}: ${ds.join(', ')}</li>`);
  html += `</ul><hr>`;

  // Common availability or detailed explanation
  if (common.length) {
    html += `<h2 class="game-title">Common Days</h2><p>${common.join(', ')}</p><hr>`;
  } else {
    html += `<h2 class="game-title">No common availability</h2>`;
    html += `<p>These players do not share any day together. Their individual available days are:</p><ul>`;
    Object.entries(votesObj).forEach(([u, ds]) => {
      html += `<li>${u}: ${ds.length ? ds.join(', ') : 'None'}</li>`;
    });
    html += `</ul><hr>`;
  }
  ...
  Object.keys(users).filter(u => !game.players.includes(u)).forEach(u => html += `<option>${u}</option>`);
  html += `</select><button>Add Player</button></form>`;

  // Back link
  html += `<a href="/" class="back-button">Back</a>`;
